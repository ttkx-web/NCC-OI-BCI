"""One-preprocess, one-Backbone, manifest-defined multi-head inference."""
...
from bci_dayloop.applications.three_mental_states.contract import (
    AWAKENING_CLASS_NAMES, AWAKENING_TASK, HeadCheckpointInfo, HeadPrediction,
    KNOWN_TASK_OUTPUT_DIMS, SHARED_FEATURE_CONTRACT, TASK_OUTPUT_DIMS, TASKS,
    ThreeMentalStateDiagnostics, ThreeMentalStatePrediction,
)
...
def _metadata_backbone_sha256(task: str, metadata: Mapping[str, Any]) -> str:
    values = {
        str(metadata[key])
        for key in ("backbone_sha256", "backbone_checkpoint_sha256")
        if metadata.get(key)
    }
    if not values:
        raise ValueError(f"{task}: checkpoint metadata has no backbone SHA256.")
    if len(values) != 1:
        raise ValueError(f"{task}: checkpoint metadata contains conflicting backbone SHA256 values.")
    return values.pop()
...
def _load_head(*, task: str, checkpoint_path: Path | str, config: Model50MConfig, class_name_fallback: Sequence[str] | None) -> _LoadedHead:
    ...
    for key, expected in SHARED_FEATURE_CONTRACT.items():
        ...
        if actual is None and task == AWAKENING_TASK:
            if key in {"embedding_layer_resolved", "embedding_layer_internal_index"}:
                actual = expected
            elif key == "backbone_adaptation" and metadata.get("training_parameters", {}).get("backbone_frozen") is True:
                actual = "frozen"
            elif key == "freeze_backbone" and metadata.get("training_parameters", {}).get("backbone_frozen") is True:
                actual = True
        if actual != expected:
    ...
    if output_dim != expected_output_dim:
        raise ValueError(f"{task}: Linear output dim expected {expected_output_dim}, actual {output_dim}.")
    ...
    if task == AWAKENING_TASK:
        if names != AWAKENING_CLASS_NAMES:
            raise ValueError(
                f"awakening: class_names must be {AWAKENING_CLASS_NAMES}, got {names}."
            )
        if int(metadata.get("positive_class_index", -1)) != 1:
...
class ThreeMentalStatePredictor:
    """Run exactly one frozen 50M feature extraction for all configured heads."""
    ...
    @classmethod
    def from_checkpoints(cls, *, backbone_checkpoint: Path | str, workload_head: Path | str, attention_head: Path | str, emotion_head: Path | str, awakening_head: Path | str | None = None, device: str = "cpu", workload_class_names: Sequence[str] | None = None, attention_class_names: Sequence[str] | None = None, emotion_class_names: Sequence[str] | None = None, awakening_class_names: Sequence[str] | None = None) -> "ThreeMentalStatePredictor":
        ...
        head_paths: dict[str, Path | str] = {
            "workload": workload_head,
            "attention": attention_head,
            "emotion": emotion_head,
        }
        fallbacks: dict[str, Sequence[str] | None] = {
            "workload": workload_class_names,
            "attention": attention_class_names,
            "emotion": emotion_class_names,
        }
        if awakening_head is not None:
            head_paths[AWAKENING_TASK] = awakening_head
            fallbacks[AWAKENING_TASK] = awakening_class_names
        return cls.from_config_and_head_checkpoints(
            config=config,
            head_checkpoints=head_paths,
            class_name_fallbacks=fallbacks,
        )
    @classmethod
    ...
    @classmethod
    def from_config_and_head_checkpoints(
        cls,
        *,
        config: Model50MConfig,
        head_checkpoints: Mapping[str, Path | str],
        class_name_fallbacks: Mapping[str, Sequence[str] | None] | None = None,
        ...
        normalized_paths: dict[str, Path | str] = {}
        for raw_task, path in head_checkpoints.items():
            task = str(raw_task).strip()
            if not task:
                raise ValueError("task_id must be a non-empty string.")
            if task in normalized_paths:
                raise ValueError(f"Duplicate task_id: {task!r}.")
            normalized_paths[task] = path
        fallbacks = class_name_fallbacks or {}
        heads = {
            task: _load_head(
                task=task,
                checkpoint_path=path,
                config=config,
                class_name_fallback=fallbacks.get(task),
            )
            for task, path in normalized_paths.items()
        }
        expected_hash = sha256_file(backbone)
        if {_metadata_backbone_sha256(task, head.info.metadata) for task, head in heads.items()} != {expected_hash}: raise ValueError("Head checkpoints do not all reference the supplied backbone: expected " + expected_hash + ".")
    ...
    def _validate_initialization(self) -> None:
        if not self._heads: raise ValueError("Multi-head predictor requires at least one head.")
        ...
        for task, loaded in self._heads.items():
            ...
            expected_output_dim = KNOWN_TASK_OUTPUT_DIMS.get(task, loaded.info.output_dim)
            if loaded.info.output_dim != expected_output_dim: raise ValueError(f"{task}: output dim expected {expected_output_dim}, actual {loaded.info.output_dim}.")
    ...
    def predict(self, window: RawEEGWindow) -> ThreeMentalStatePrediction:
        ...
        prediction = {task: self._prediction(task, logits[task], head.info.class_names) for task, head in self._heads.items()}
        ...
        self._last_diagnostics = ThreeMentalStateDiagnostics(1, 1, {task: 1 for task in self._heads}, tuple(preprocessed.signal.shape), tuple(embedding.shape), tuple(feature.shape), {task: tuple(value.shape) for task, value in logits.items()}, preprocessed.mapped_channel_count, preprocessed.missing_channel_count, missing, preprocessed.unknown_channel_names, preprocessed.duplicate_channel_count, prep_ms, backbone_ms, heads_ms)
