...
基于公司 50M EEG 基座模型的离线、伪实时与实时脑机接口基础设施。

项目提供统一的 EEG 数据读取、预处理、模型适配、Runtime Model Package、滑窗推理、运行控制、指标记录和 Streamlit 可视化能力。

当前状态：Stage 0.5 已完成，正在并行推进 Stage 1 简单个体化 与 Stage 2A 公司自采离线 EEG 数据兼容。

## Current Status

| Stage | Description | Status |
|---|---|---|
| 0 | LaBraM pseudo-realtime baseline | Completed |
| 0.5 | 50M integration and validation | Completed |
| 1 | Simple personalization | In progress |
| 2A | Self-collected offline EEG compatibility | In progress |
| 2B | Realtime EEG device integration | Planned |
| 3 | Realtime pipeline with personalization | Planned |
| 3.5 | Rest-Tuning | Planned |
| 4 | Online adaptation | Planned |

## Current Runtime Model

当前默认运行模型为公司 50M EEG 基座模型。

Stage 0 使用的 LaBraM Pipeline 作为历史基线继续保留，
用于回归测试和模型替换前后的工程对照，但不再作为默认正式模型。

## 50M Model Adapter

阶段 0.5 的 50M 模型适配说明见：

- [50M Model Adapter README](README_50M_MODEL_ADAPTER.md)
