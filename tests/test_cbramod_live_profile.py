    ...
    class FakeBackend:
        ...
        def predict_tensor(
            ...
            *,
            ...
            batch_size = int(model_input["signal"].shape[0])
            probabilities = torch.full((batch_size, 4), 0.25)
            return ModelOutput(
...
def test_loader_rejects_legacy_allow_missing_channels_package(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(ValueError, match="allow_missing_channels=true"):
...
def test_sequential_live19_package_loads_policy_and_predicts(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    profile, config = _profile_config(
    ...
    metadata = SequentialDatasetMetadata(
        ...
        unit="uV",
        dataset_name="synthetic_live19",
        window_sec=4.0,
    )
    dataset = SequentialDataset(
        metadata=metadata,
        data=np.ones((1, 22, 800), dtype=np.float32),
        labels=np.asarray([0], dtype=np.int64),
        subject_ids=np.asarray([1], dtype=np.int64),
        session_ids=np.asarray(["0train"], dtype=str),
        trial_ids=np.asarray(["trial-1"], dtype=str),
        trial_ordinals=np.asarray([0], dtype=np.int64),
        window_ids=np.asarray(["window-1"], dtype=str),
    )
    preprocessor = CBraModPipelinePreprocessor(config)
    completion_sha = preprocessor.completion_matrix_sha256_for(
        profile.observed_channel_names
    )
    prepared_trials = prepare_cbramod_trials(
        data=dataset.data,
        ...
        observed_channel_names=profile.observed_channel_names,
        simulated_missing_channels=profile.simulated_missing_channels,
        expected_completion_matrix_sha256=completion_sha,
    )
    assert prepared_trials.shape == (1, 22, 4, 200)

    package_payload, classifier_metadata = _package_payload(
        tmp_path / "live19",
        spline=True,
    )
    preprocessing = package_loader.load_yaml(
        tmp_path / "live19" / "preprocessing.yaml"
    )
    preprocessing["transform"]["completion_matrix_sha256"] = completion_sha
    package_payload["runtime"]["channel_completion"]["completion_matrix_sha256"] = completion_sha
    package_payload["provenance"]["completion_matrix_sha256"] = completion_sha
    classifier_metadata["completion_matrix_sha256"] = completion_sha
    dump_yaml(preprocessing, tmp_path / "live19" / "preprocessing.yaml")
    dump_yaml(package_payload, tmp_path / "live19" / "package.yaml")
    _patch_cbramod_loader(monkeypatch, classifier_metadata)

    loaded = package_loader.load_runtime_package(
        tmp_path / "live19", device="cpu", verify_hashes=False
    )
    assert (
        loaded.runtime_model.input_transform.completion_matrix_sha256_for(
            profile.observed_channel_names
        )
        == completion_sha
    )
    RealtimeModelPolicyRegistry.create(loaded)
    prediction = loaded.runtime_model.predict(
        RawEEGWindow(
            data=dataset.data[0, :19],
            channel_names=list(profile.observed_channel_names),
            sample_rate=200.0,
            unit="uV",
            layout="CT",
    ...
    assert prediction.probabilities.shape == (1, 4)
